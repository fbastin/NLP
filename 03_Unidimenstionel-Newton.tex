...
tout $\xi \in \cB(x^*, \epsilon_1) := \{ x \,|\, |x - x^*| < \epsilon_1 \}$, $|f'''(\xi)| < k_1$ et $|f''(\xi)| > k_2$.
...
Si $| x_k - x^*| < \epsilon$, $x_k \in \cB(x^*, \epsilon_1)$, et
$
| f'''(x_k) | < k_1 \quad | f''(x_k) | > k_2. 
$

Autrement dit, dans ce cas, $\exists \gamma \in \cB(x^*,\epsilon_1)$ tel que
\begin{align*}
& f'(x^*) = f'(x_k) + f''(x_k)(x^*-x_k)+\frac{f'''(\gamma)}{2}(x^*-x_k)^2 \\
...
La méthode de Newton est également utilisée pour déterminer un point où la fonction s'annule.
