...
\begin{frame}
\frametitle{LICQ et condition de Slater}

Dans le cas \textbf{convexe}, la condition de qualification naturelle est la condition de Slater.
Un lien important relie la LICQ (condition locale) à Slater (condition globale).

\mbox{}

\begin{thm}
Considérons un programme convexe avec contraintes d'inégalité linéaires
$$
g_i(x) = a_i^T x - b_i \leq 0,\quad i = 1,\ldots,m.
$$
Si la LICQ tient en un point réalisable $x^*$, alors la condition de Slater est satisfaite,
i.e.\ il existe $\bar{x}$ tel que $g_i(\bar{x}) < 0$ pour tout $i = 1,\ldots,m$.
\end{thm}

\mbox{}

{\blue Remarque}: la LICQ est une condition locale (en $x^*$), tandis que Slater est
une condition globale (existence d'un point intérieur).
Le résultat ci-dessus vaut pour des contraintes linéaires ; il ne s'étend pas en général
aux contraintes non-linéaires.

\end{frame}

\begin{frame}
\frametitle{LICQ implique Slater: preuve}

\begin{proof}
Notons $\cA(x^*) = \{i \mid g_i(x^*) = 0\}$ les contraintes actives en $x^*$, et $A$
la matrice dont les lignes sont $a_i^\top$, $i \in \cA(x^*)$.

\mbox{}

La LICQ signifie que $A$ est de rang plein, donc $AA^\top \succ 0$.
Choisissons $d = -A^\top(AA^\top)^{-1}\mathbf{1}$, de sorte que $Ad = -\mathbf{1}$,
i.e.\ $a_i^T d = -1 < 0$ pour tout $i \in \cA(x^*)$.
Posons $\bar{x} = x^* + \epsilon d$ pour $\epsilon > 0$ suffisamment petit.

\begin{itemize}
\item \textbf{Contraintes actives} ($i \in \cA(x^*)$) : les contraintes étant linéaires,
$$
g_i(\bar{x}) = \underbrace{g_i(x^*)}_{=0} + \epsilon\, a_i^T d = -\epsilon < 0.
$$
\item \textbf{Contraintes inactives} ($i \notin \cA(x^*)$) : $g_i(x^*) < 0$, et par
linéarité $g_i(\bar{x}) = g_i(x^*) + \epsilon\, a_i^T d < 0$ pour $\epsilon$ assez petit.
\end{itemize}

Donc $g_i(\bar{x}) < 0$ pour tout $i$ : la condition de Slater est satisfaite. \qed
\end{proof}
