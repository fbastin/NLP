...
\begin{thm}
Supposons que le Lagrangien associé au problème précédent
possède un minimum local $x^* \in \cX$ lorsque le vecteur de multiplicateurs $\lambda$ vaut $\lambda^*$.
...
& h_j(x) = 0,\ j = 1,\ldots,r,
\end{align*}
où les fonctions $f$, $g_i$, $i = 1,\ldots,m$, et $h_j$, $j = 1,\ldots,r$, sont continûment différentiables, et qu'une condition de qualification de contrainte tient en $x^*$.
...
\begin{frame}
	\frametitle{Inversibilité de $AH^{-1}A^T$ et complément de Schur}

	La matrice $AH^{-1}A^T \in \RR^{p \times p}$ est inversible sous nos hypothèses.
	En effet, pour tout $v \ne 0$,
	$$
	v^T AH^{-1}A^T v = (A^Tv)^T H^{-1} (A^Tv).
	$$
	Puisque $H$ est définie positive, $H^{-1}$ l'est aussi, donc $(A^Tv)^T H^{-1}(A^Tv) > 0$ dès que $A^Tv \ne 0$.
	Or $A \in \RR^{p \times n}$ est de rang plein (rang $p$), donc $A^T$ est injective: $A^Tv = 0 \Rightarrow v = 0$.
	Dès lors $AH^{-1}A^T$ est définie positive, en particulier inversible.

	\medskip
	\textbf{Lien avec le complément de Schur.}
	La matrice de selle du système KKT est
	$$
	K = \begin{pmatrix} H & A^T \\ A & 0 \end{pmatrix}.
	$$
	Le \textbf{complément de Schur} de $H$ dans $K$ est $S = 0 - AH^{-1}A^T = -AH^{-1}A^T$.
	Puisque $S$ est définie négative (et donc inversible), $K$ est elle-même inversible, ce qui garantit l'existence et l'unicité de la solution $(x^*, \lambda^*)$.
...
\begin{lem}
	...
	d^T \nabla g_i(x^*) &\leq 0,\ i \in \cA(x^*) \cap \cI
	\end{align*}
	Sous ces hypothèses, il existe un chemin réalisable issu de $x^*$ de direction tangente $d$ (énoncé complet au frame suivant).
\end{lem}

\end{frame}

\begin{frame}
\frametitle{LICQ: lemme (suite)}
