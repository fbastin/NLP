...
Une fonction est dite \textcolor{blue}{unimodale} si elle possède un seul mode, c'est-à-dire un et un seul point où elle atteint un maximum local, dans le cas d'un problème de maximisation, ou un et un seul point où elle atteint un minimum local, dans le cas d'un problème de minimisation.
...
Soit $f: [a, b] \rightarrow \RR$ unimodale. Considérons le problème
$$
\min_{x \in [a,b]} f(x),
$$
...
Il est aussi possible de montrer (formule de Binet) que
$$
...
$$
d_{k+1} = d_k-\frac{F_{N-k-1}}{F_{N-k+1}}d_k
= \frac{F_{N-k+1} - F_{N-k-1}}{F_{N-k+1}}d_k = \frac{F_{N-k}}{F_{N-k+1}}d_k,
$$
la dernière égalité résultant de la définition des nombres de Fibonacci.
...
\begin{frame}
\frametitle{Longeur d'intervalle}
...
$$
d_k = \frac{F_{N-k+1}}{F_N}d_1,\ k = 1, 2,\ldots, N
$$

Celle peut être prouvée par récurrence.
Elle est triviale pour $k = 1$:
$$
d_1 = \frac{F_N}{F_N}d_1 = d_1.
$$
Supposons à présent
$$
d_{k} = \frac{F_{N-k+1}}{F_N}d_1.
$$
Alors,
$$
d_{k+1} = \frac{F_{N-k}}{F_{N-k+1}}d_k = \frac{F_{N-k}}{F_{N-k+1}}\frac{F_{N-k+1}}{F_N}d_1
= \frac{F_{N-k}}{F_N}d_1.
$$
...
En effet, du résultat précédant, nous avons
%puisqu'en utilisant cette stratégie de sélection des points d'évaluation, nous avons
%$$
%d_k = \frac{F_{N-k+1}}{F_N}d_1,\ k = 1, 2,\ldots, N
%$$
%il s'ensuit que
\begin{align*}
d_{N-1} &= \frac{F_{N-(N-1)+1}}{F_N}d_1 = \frac{F_2}{F_N}d_1 = \frac{2}{F_N}d_1, \\
...
Soit $f: \RR \rightarrow \RR$, dérivable en $c$:
$$
...
\begin{align*}
f(x) < f(c) \quad \forall x \in (c-\delta, c)\\
f(c) < f(x) \quad \forall x \in (c, c+\delta)\\
\end{align*}
...
Si $f: \RR \rightarrow \RR$, et $f'(c) = 0$, $c$ n'est pas nécessairement un minimum ou un maximum local.
...
\begin{thm}
Soit $f: \RR \rightarrow \RR$ deux fois dérivable en $c$ et telle que $f'(c) = 0$.
...
Soit $f: S \rightarrow \RR$, avec $S \subset \RR$.
...
Considérons par exemple $f: [-1,1] \rightarrow \RR$, $f(x) = x^3$. Le minimum est atteint en $x^* = -1$, mais $f'(-1) \ne 0$.
